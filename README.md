# Lab: Taller Mecánico — Agenda de Talleres y Servicios

## 📖 Descripción del Laboratorio

Modelado y gestión de una agenda de talleres mecánicos con datos de contacto y catálogos de servicios ofrecidos mediante listas de cadenas.

---

## 🎯 Objetivos y Requisitos Técnicos

- [x] Clase `Taller` con atributos complejos (`ArrayList<String> servicios`)
- [x] Métodos específicos de presentación resumida y detallada
- [x] Instanciación múltiple y búsqueda de talleres específicos
- [x] Encapsulamiento y buenas prácticas

---

## 🛠️ Tecnologías y Herramientas

- **Java 17+**
- **IntelliJ IDEA**
- **POO y Colecciones**

---

## 🚀 Cómo Ejecutar el Proyecto

```bash
# 1. Clonar el repositorio
git clone https://github.com/albertguiteras/tallerMecanico.git

# 2. Acceder al directorio de fuentes
cd tallerMecanico/src

# 3. Compilar los archivos Java
javac *.java

# 4. Ejecutar la clase principal
java Main
```
